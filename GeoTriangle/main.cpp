...
#include "CompiledShaders/GeometryShader.h"
...
public:
	...
	GLuint InitProgram( const uint32_t* VertexShaderBinary, GLsizei VertexShaderBinaryLength, const uint32_t* GeometryShaderBinary, GLsizei GeometryShaderBinaryLength, const uint32_t* FragmentShaderBinary, GLsizei FragmentShaderBinaryLength )
	{
		...
		result &= InitShader( GL_GEOMETRY_SHADER, GeometryShaderBinary, GeometryShaderBinaryLength, geomShader, "GeometryShader" );
	...
	void loadAssets()
	{
		Program = InitProgram( VertexShaderBinary, sizeof(VertexShaderBinary), GeometryShaderBinary, sizeof(GeometryShaderBinary), FragmentShaderBinary, sizeof(FragmentShaderBinary) );
	}
	...
	void generateBuffers()
	{
		...
		glObjectLabel( GL_VERTEX_ARRAY, VAO, -1, "EmptyVAO" );
	...
	void draw(double deltaT)
	{
		...
		{
			static int DrawErrorsPrinted = 0;

			if ( ++DrawErrorsPrinted<30 )
			{
				if ( DrawError==GL_INVALID_OPERATION )
				{
					std::cerr << "DrawError: GL_INVALID_OPERATION" << std::endl;
				}
				else
				{
					std::cerr << "DrawError: " << std::hex << DrawError << std::endl;
				}
			}
		}
